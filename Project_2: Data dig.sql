...
/* what is the most expensive price tagged? */

SELECT MAX(price) AS the_most_paid_product FROM sales; 

/* what is the avg price that one pays in the store for all furniture? */

SELECT AVG(price) AS avg_price_point_of_most_products_in_store FROM sales;

/* what is the total sum transaction from all sales? */

SELECT SUM(price) AS total_amount_paid FROM sales;

/* select the total transaction sales that payed over 2 thousands? */

SELECT COUNT(*) AS more_than_2k FROM sales WHERE price > 1200; 

/* select the top 3 countries that have the most sales? */

SELECT country, COUNT(*) as top_3_selling_countries FROM sales GROUP BY country HAVING top_3_selling_countries > 9;

   
SELECT country, CASE
    WHEN country = "United states" THEN "domestic sale"
    ELSE "foreign sale"
    END AS "sales_made"
    FROM sales;
    
Select count(country) as percent, case
when country="United states" then "Domestic sale" 
else "foreign sale"
end "sales_made"
from sales
...
SELECT transaction_date FROM sales WHERE state = "CA" AND price = 1200; 

SELECT transaction_date FROM sales WHERE state = "CA" OR price = 3600;

SELECT payment_type FROM sales GROUP BY payment_type;

SELECT city, COUNT(*) AS most_sold_store FROM sales GROUP BY city HAVING most_sold_store > 1;
